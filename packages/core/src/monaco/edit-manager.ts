...
export class EditManager {
  ...
  constructor(monaco: MonacoNamespace, editor: MonacoStandaloneEditor) {
    this.monaco = monaco
    this.editor = editor
  }
  ...
  nextEdit(): void {
    ...
    this.currentEditIndex =
      (this.currentEditIndex + 1) % this.pendingEdits.edits.length
    this.renderPreview()
  }
  ...
  private renderPreview(): void {
    ...
    const decorations: MonacoModelDeltaDecoration[] = []
    ...
    this.pendingEdits.edits.forEach((edit, index) => {
      ...
      decorations.push({
        ...
        options: {
          className: isCurrent
            ? "monaco-autocomplete-edit-current"
            : "monaco-autocomplete-edit",
          hoverMessage: {
            value: isCurrent
              ? `**Edit ${index + 1}/${this.pendingEdits!.edits.length}** (Press Tab to apply)`
              : `Edit ${index + 1}`,
          },
        },
      })
